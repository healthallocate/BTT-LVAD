...
cand_list = cand_thor %>% 
  ...
  filter(CAN_LAST_ACT_STAT_DT  >= filter_date_start) %>%
...
risk_strat = risk_strat %>%
  ...
  # # keep track of which status was requested by this JustId and by which criteria
  # mutate(RequestedCandStatCd_criteria = status_criteria) %>%
  # group_by(PX_ID, JustId) %>% 
  # fill(RequestedCandStatCd,.direction="downup") %>%
  # fill(RequestedCandStatCd_criteria, .direction = "downup") %>%
...
full_list = full_list %>% 
  ...
  group_by(PX_ID) %>%
  fill(systolicBP:CANHX_STAT_CD, .direction = "down") %>% # note: not including CANHX_REASON_STAT_INACT
  ungroup() %>%
  ...
  # this fills in missing values using the na.locf() method
  group_by(PX_ID, JustId) %>%
  fill(status_criteria:ExtInotropeDopamine, .direction = "down") %>% # status_criteria and RequestedCandStatCd are filled "down" but grouped by JustId which means the fill can stop (same for other variables here and dose variables below)
  ungroup() %>%
...
# no more missing statuses
table(df$CANHX_STAT_CD,useNA="ifany")
...
# confirm these are constant across px_id
df %>% group_by(PX_ID) %>% filter(n_distinct(transplant_time)!=1)
df %>% group_by(PX_ID) %>% filter(n_distinct(death_time)!=1)
df %>% group_by(PX_ID) %>% filter(n_distinct(removal_time)!=1)
  
df = df %>% filter(death_date_max >= filter_date_start)
```
...
df = df %>% group_by(PX_ID) %>%
  mutate(transplant = cumsum(cumsum(transplant)),
         # follow to last unique date after transplant: REC_DISCHRG_DT, TFL_LAST_FU_DT, death date, TFL_GRAFT_DT
         t_stop = ifelse(transplant == 1, last(t_stop), t_stop))

df %>%
  filter(transplant > 1,row_number()==n()) %>%
  ungroup() %>%
  count(unique_event) %>% as.data.frame
...
save(df,file="heart_progress.RData")

# what's going on when t_start = t_stop?

## some of these are recent transplants w/ no follow-up visit yet, OR transplant recipients who died on date of transplant
df %>% filter(transplant==1,t_start==t_stop) %>% select(PX_ID,unique_event,unique_date,t_start,t_stop,transplant,outcome)

## also, delistings that died on date of delisting or have not died yet
df %>% ungroup() %>% filter(grepl("CAN_REM_DT",unique_event),transplant==0,t_start==t_stop) %>%
  select(PX_ID,unique_event,unique_date,t_start,t_stop,transplant,outcome)

# these delistings are always the last row
df %>% filter(any(grepl("CAN_REM_DT", unique_event) & transplant==0 & t_start==t_stop & row_number()!=n())) %>% nrow()

# what's going on with rows after delisting when there was no death?
## note: these censoring times are just data byproducts, eg ChangeDt after removal
df %>% filter(any(grepl("CAN_REM_DT", unique_event) & transplant==0 & t_start!=t_stop & outcome!=1),t_start>=removal_time) %>%
  select(PX_ID,unique_event,unique_date,t_start,t_stop,transplant,outcome,removal_time,death_time)

# look at a few specific ids
df %>% filter(PX_ID %in% c(1471218,1526025,1337428)) %>%
  slice(n()-2,n()-1,n()) %>%
  select(PX_ID,unique_event,unique_date,t_start,t_stop,transplant,outcome,removal_time,death_time)
...
table(df$transplant==1,useNA="ifany")
# 15516

# how many deaths? including some posttransplant deaths from OPTN
df %>% filter(transplant<=1,outcome==1) %>% distinct(PX_ID) %>% nrow()
# 3224

# how many delistings?
df %>% filter(transplant==0,outcome==0,grepl("CAN_REM_DT",unique_event)) %>% nrow()
# 2249

## WARNING - This is removing delistings w/o death dates (delisting=1, transplant=0, outcome=0) that we want to follow after removal 
tmp1 = df %>% filter(transplant <= 1 & t_start != t_stop) %>% 
  ungroup()

# instead, keep rows that correspond to a transplant or delisting (not for death or tx) even if t_start=t_stop
tmp2 = df %>% filter(transplant <= 1 &
                      ((t_start==t_stop & transplant==1) |
                         (t_start==t_stop & grepl("CAN_REM_DT",unique_event) & transplant==0 & outcome==0) |
                         (t_start!=t_stop) |
                         n() == 1))
dim(table(tmp2$PX_ID)) # 21100
table(tmp2$transplant) # 15516
table(tmp2$outcome) # 3241 - too many
tmp2 %>% filter(transplant==0,outcome==0,grepl("CAN_REM_DT",unique_event)) %>% nrow() # 2249

# problem: a person can have multiple deaths, if death was on same day as transplant
tmp2 %>% filter(sum(outcome)>1) %>%
  slice(n()-1,n()) %>%
  select(PX_ID,unique_event,unique_date,t_start,t_stop,transplant,outcome,removal_time,death_time)

# make only one death per px_id
tmp2 = tmp2 %>%
  mutate(outcome = ifelse(sum(outcome)>1 & outcome==1 & row_number() != n() & t_stop==lead(t_stop),0,outcome))
table(tmp2$outcome,useNA="ifany") # 3224

# need to get rid of rows after outcome=1, without losing transplants
tmp2 %>% mutate(outcome_cs=cumsum(cumsum(outcome))) %>% filter(any(outcome_cs>1)) %>%
  slice(n()-1,n()) %>%
  select(PX_ID,unique_event,unique_date,t_start,t_stop,death_date_max,death_time,outcome,outcome_cs,transplant)

# get rid of rows after outcome=1 unless transplant=1
tmp2 = tmp2 %>% 
  filter(cumsum(cumsum(outcome))<=1 | transplant==1) %>% 
  ungroup()
dim(table(tmp2$PX_ID)) # 21100
table(tmp2$transplant) # 15516
table(tmp2$outcome) # 3224
tmp2 %>% filter(transplant==0,outcome==0,grepl("CAN_REM_DT",unique_event)) %>% nrow() # 2248 now, instead of 2249
...
# indicator of BNP NT Pro
df = df %>%
  mutate(BNP_NT_Pro = ifelse(HrSevFailNtBnpType=='NT Pro BNP',1,0))
df %>% filter(!is.na(BNP),is.na(HrSevFailNtBnpType)) %>% nrow()
